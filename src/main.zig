...
const State = struct {
    ...
    fn init(self: *State) !void {
        self.window = c.SDL_CreateWindow("Draw", SCREEN_WIDTH * shared.SCALE, SCREEN_HEIGHT * shared.SCALE, c.SDL_WINDOW_MOUSE_CAPTURE).?;
    ...
    fn render(self: *State) void {
        ...
        @memcpy(pixels, @as([]u8, @ptrCast(draw.drawn_pixels.pixels)));

        // draw cursor
        fillCircle(pixels[0 .. SCREEN_WIDTH * SCREEN_HEIGHT * shared.BPP8], @intFromFloat(curr_mouse_point.x), @intFromFloat(curr_mouse_point.y), draw.pencil_radius, 0x00888888);

        if (c.SDL_MUSTLOCK(self.surface)) {
            _ = c.SDL_UnlockSurface(self.surface);
        }

        const scaled_surface = c.SDL_ScaleSurface(self.surface, SCREEN_WIDTH * shared.SCALE, SCREEN_HEIGHT * shared.SCALE, c.SDL_SCALEMODE_NEAREST);
...
pub fn main() !void {
    ...
    mainloop: while (true) {
            ...
            switch (e.type) {
                ...
                c.SDL_EVENT_MOUSE_BUTTON_DOWN => {
                    ...
                    switch (e.button.button) {
                        c.SDL_BUTTON_LEFT => mouse_button_down = shared.MouseButton.Left,
                        c.SDL_BUTTON_RIGHT => mouse_button_down = shared.MouseButton.Right,
                        else => {},
                    }
                },
                c.SDL_EVENT_MOUSE_BUTTON_UP => mouse_button_down = shared.MouseButton.None,
                c.SDL_EVENT_MOUSE_WHEEL => {
                    if (e.wheel.y < 0) {
                        draw.pencil_radius = min(draw.pencil_radius + 1, draw.PENCIL_MAX_RADIUS);
                    } else if (e.wheel.y > 0) {
                        draw.pencil_radius = max(if (draw.pencil_radius > 0) draw.pencil_radius - 1 else 0, draw.PENCIL_MIN_RADIUS);
                    }
                },
                else => {},
            }
        ...
        draw.update(mouse_button_down, curr_mouse_point);
        state.render();
    }
...
fn colorPixel(mem: []u8, x: usize, y: usize, color: u32) void {
    const index = ((y * SCREEN_WIDTH) + x) * shared.BPP8;
    if (index < 0 or index >= SCREEN_WIDTH * SCREEN_HEIGHT * shared.BPP8) return;

    const pixel = shared.Pixel{
...
fn fillSegment(mem: []u8, p1: shared.PointF, p2: shared.PointF, r: usize, color: u32) void {
